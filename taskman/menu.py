...
def priority_ask(): #запросить и вернуть проверенный приоритет задачи
    while True:
        raw = io_utils.ask_int('приоритет (3): ', default=3)
        if raw is None:
...
def card_print(task: Task):#напечатать подробную карточку задачи
    ...
    print(f'приоритет: {priority} ({label})')
    print(f'срок: {task.get("due")}')
...
def cmd_stats(tasks: TaskList): #показать статистику по задачам
    data = storage.stats(tasks)
    print(f'всего: {data["total"]}, выполнено: {data["done"]}, '
          f'осталось: {data["left"]}, средний приоритет: {data["avg_priority"]:.1f}')
