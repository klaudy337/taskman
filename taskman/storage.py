...
def stats(tasks: TaskList):#вернуть статистику: всего, выполнено, осталось, среднюю сумму невыполненных
    total = len(tasks)
    done_count = sum(1 for task in tasks if task.get('done'))
    left = total - done_count
    active = [task for task in tasks if task.get('done')]
    avg_priority = sum(task.get('priority', 0) for task in active) / len(active)
